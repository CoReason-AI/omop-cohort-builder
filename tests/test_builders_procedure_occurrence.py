...
def test_procedure_occurrence_modifier_cs():
    ...
    criteria = ProcedureOccurrence(modifier_cs=ConceptSetSelection(codeset_id=300))
    query = qb.build_criteria(criteria)
    sql = normalize_sql(compile_query(query))
...
def test_procedure_occurrence_age():
    qb = QueryBuilder()
    criteria = ProcedureOccurrence(age=NumericRange(value=50, op="gt"))
    query = qb.build_criteria(criteria)
    sql = normalize_sql(compile_query(query))
    ...
    assert (
        "EXTRACT(year FROM procedure_occurrence.procedure_date) - person.year_of_birth > 50"
        in sql
    )
...
def test_procedure_occurrence_gender_cs():
    ...
    criteria = ProcedureOccurrence(gender_cs=ConceptSetSelection(codeset_id=400))
    query = qb.build_criteria(criteria)
    sql = normalize_sql(compile_query(query))
...
def test_procedure_occurrence_visit_type():
    ...
    criteria = ProcedureOccurrence(visit_type_cs=ConceptSetSelection(codeset_id=600))
    query = qb.build_criteria(criteria)
    sql = normalize_sql(compile_query(query))
